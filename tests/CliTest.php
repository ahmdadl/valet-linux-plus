...
class CliTest extends TestCase
{
    ...
    public function itWillHandleInvalidPhpVersion(): void
    {
        ...
        $content = $output->fetch();
        $this->assertStringContainsString(
            'Invalid version [7.2] used. Supported versions are: 8.2 or later',
            $content
        );
        $this->assertStringContainsString(
            'You can still use any version from [',
            $content
        );
        $this->assertStringContainsString(\Valet\PhpFpm::MIN_ISOLATION_VERSION, $content);
        $this->assertStringContainsString(\Valet\PhpFpm::MIN_SUPPORTED_VERSION, $content);
    }
